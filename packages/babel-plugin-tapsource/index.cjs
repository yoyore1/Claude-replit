  ...
  return {
    ...
    visitor: {
      JSXOpeningElement(nodePath, state) {
        ...
        // Don't tag the shared UI kit (or any excluded path). Kit internals are
        // edited via tap-editable PROPS (see tapField), never by rewriting the
        // kit source — tagging them would let a tap resolve into a shared file
        // and change every screen/app at once. Skipping them makes every tap
        // resolve to the user's authored element (App.tsx), keeping edits
        // independent. `exclude` is a list of POSIX path prefixes.
        const exclude = state.opts.exclude || [];
        if (exclude.some((p) => rel === p || rel.startsWith(p))) return;
